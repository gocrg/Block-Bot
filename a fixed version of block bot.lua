...
local Config = {
    ...
    -- (natural selection) username/displayname
...
-- Updated to support username & display name
...
local function GetFollowPosition(cf, dist)
    return Config.FollowPosition:lower() == "back" and CalculatePositionBehind(cf, dist) or CalculatePositionInFront(cf, dist)
end
...
local function CheckIfStuck()
    ...
    if (currentPosition - LastPosition).Magnitude < 1 then
        if os.time() - LastPositionTime > Config.StuckTimeout then
            ...
            if Config.JumpWhenBlocked then Humanoid:ChangeState(Enum.HumanoidStateType.Jumping) end
            if Config.RespawnIfStuck then Respawn() end
...
local function FollowTarget()
    ...
    local targetPos, botPos = targetHRP.Position, HumanoidRootPart.Position

    if Config.TeleportIfTooFar and (targetPos - botPos).Magnitude > Config.TooFarDistance then
        HumanoidRootPart.CFrame = CFrame.new(GetFollowPosition(targetHRP.CFrame, Config.FollowDistance))
        Notify("Teleported to target")
        return true
    end

    if Config.RespawnIfStuck then CheckIfStuck() end

    if not Config.SmoothMovement then
        Humanoid:MoveTo(GetFollowPosition(targetHRP.CFrame, Config.FollowDistance))
    end

    return true
end

local function RenderSteppedFollow(dt)
    if IsFollowing and Config.SmoothMovement then
        if not TargetCharacter or not TargetCharacter:FindFirstChild("HumanoidRootPart") then return end
        if not HumanoidRootPart then return end

        local targetHRP = TargetCharacter.HumanoidRootPart
        local desiredPos = GetFollowPosition(targetHRP.CFrame, Config.FollowDistance)
        local direction = (desiredPos - HumanoidRootPart.Position).Unit
        local interpolated = HumanoidRootPart.Position + (direction * Config.MovementSpeed * dt * Config.SmoothnessFactor)
        Humanoid:MoveTo(interpolated)
    ...
    while attempts < Config.MaxAttempts do
        ...
        if TargetPlayer then
            ...
            if TargetCharacter:FindFirstChild("HumanoidRootPart") then
                ...
                table.insert(ConnectionTable, RunService.Heartbeat:Connect(function()
